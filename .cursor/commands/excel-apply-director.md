# Excel Apply Director - ディレクター修正指示の適用

Excelファイルの「ディレクター確認欄」に記入された修正指示を読み取り、サイトに反映します。

## Context Files

- `Site-Fix/input/` (Excelファイルの検索対象)
- `app/components/` (修正対象のコンポーネント)
- `memories/excel_apply_director_workflow.yaml`

## Instructions

**🚨 重要: Excelファイルの「ディレクター確認欄」（E列）の修正指示を読み取り、サイトを修正します。**
...
1. Pythonスクリプトで以下を実行:
   ...
   for row in ws.iter_rows(min_row=7, values_only=True):
       if len(row) >= 5:
           page, section, content, revision, director_revision = row[:5]
           if director_revision and str(director_revision).strip():
               print(f'【ページ】{page}')
               print(f'【セクション】{section}')
               print(f'【ディレクター修正】{director_revision}')
   ```
2. ディレクター修正指示がない場合は「ディレクター修正指示がありません」と表示して終了
...
```
1. /excel-guide でExcel生成
   ↓
2. クライアントがD列（修正内容）に記入
   ↓
3. /excel-apply でD列の修正を反映
   ↓
4. ディレクターがE列（ディレクター確認欄）に追加指示を記入
   ↓
5. /excel-apply-director でE列の修正を反映
```
